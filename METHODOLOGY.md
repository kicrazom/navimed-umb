...
The campaign straddles **two host-firmware states**, established from the per-run records rather than assumed. Firmware per data regime:

- **Phase 1 (hardware envelope)** and the **Phase 2 concurrency ladder** (N ∈ {10..1000}) — the dataset reported in Paper #1 — were collected under **BIOS 1715 / AGESA ComboAm5PI 1.2.0.3g**. That campaign ran from late April to early June 2026 (earliest `thermal-runs` artifact 2026-04-26; latest pre-update envelope/ladder artifacts 2026-06-09; the concurrency ladders themselves 2026-05-21…06-04), entirely **before** the 2026-06-13 flash, and was produced by runners that did **not** yet capture a `bios_version` field (§3.3); its firmware is therefore established from the collection dates being wholly pre-update, not read from the record. (The `benchmarks/results/hardware_envelope/` tree additionally holds later **post-update** additions — e.g. three AWQ max-context envelope probes dated 2026-07-02, part of the §5.3 ablation pre-check — which are under **2202**, not 1715; firmware is per-record there, so date of collection is the discriminator, and the pre-update / post-update split must be made per file, not per directory.)
- The **N = 1 single-stream anchor** (§5.2, §7.4) was collected *after* the update, under **BIOS 2202 / AGESA ComboAm5PI 1.3.0.1**: the small/mid models on 2026-06-22 (`<model>/n1-anchor/`) and the Llama-PLLuM-70B family on 2026-06-24…26 (`<model>/scaling-n1/rep*/`). The 70B anchor records carry the firmware live per run — `bios_version = 2202`, `bios_date = 04/09/2026` (DMI release date of the 2202 image), `agesa_version = AGESA ComboAm5PI 1.3.0.1` (80 records: 8 variants × 10 reps, verified in `results_table.csv`). The small/mid anchor was produced by an earlier runner that did not aggregate the firmware field, so its 2202 status is established from its post-update collection date.

The platform BIOS was flashed 1715 → 2202 on **2026-06-13** — i.e. *after* the envelope and the concurrency ladder were already collected, **not** (as an earlier version of this section stated) in a clean window before Phase 2. **The study therefore does not report a single uniform host-firmware platform: the concurrency ladder / plateau is under 1715 and the single-stream anchor under 2202.** Envelope quantities (load success, VRAM footprint, KV-cache capacity, max-concurrency) are determined by the model–quantization–vLLM/ROCm stack and are **firmware-independent**; only host-platform firmware (CPU microcode, DDR5 training) changed across the boundary — the R9700 GPU VBIOS (`113-R9700AT-F40`) and the compute path are unchanged. But because the single-stream anchor and the concurrency plateau were measured on opposite sides of this boundary, **the single-stream-to-plateau envelope ratio compares across two firmware states** and must be reported with that provenance caveat (Paper #1 methods §2, discussion §5.5).

> **Later re-runs.** From the N = 1 anchor runner (late 2026-06) onward, host firmware is recorded live per run (§3.3); post-update work carries `bios_version = 2202` in-record rather than by inference. The Tier-A v0.5 re-runs and the precision-ablation sub-study (§5.3) execute on the updated (2202) platform. **Open max-rigor control:** re-measure one Phase-1 envelope cell on 2202 to document directly that the envelope is unchanged across the boundary.
...
Every benchmark JSON record captures the full software stack version triple (`rocm_version`, `vllm_version`, `torch_version`, `torch_hip_version`) plus the full env-var dictionary. Host-firmware capture (`bios_version`, `bios_date`, `agesa_version`; read live at run time from sysfs DMI and fwupd) was **added with the N = 1 anchor runner (late 2026-06)** and is present in every record from that point on (the N = 1 anchor, the Tier-A v0.5 re-runs, and the precision-ablation sub-study §5.3). Records from earlier runners — the Phase-1 envelope and the early-June Phase-2 v0.3 concurrency ladder — do **not** carry the firmware fields; their firmware is established from collection date (wholly pre-2026-06-13 → BIOS 1715), as documented in the §2.1 platform version boundary. Any reproduction attempt that does not reproduce this triple is not a reproduction; it is a different experiment. This requirement follows Lerchner (2026) — computation as description requires complete vehicle specification, of which platform firmware is a part.
...
| # | Model | Params | Quant | TP | Status | Notes |
|---|---|---|---|---|---|---|
| 12 | `CYFRAGOVPL/Llama-PLLuM-8B-instruct` | 8B | BF16 | 1 | ready | Llama base. AWQ-4bit variant `mozarcik/Llama-PLLuM-8B-chat-2512-awq` published 2026-05-26 (Run-3, consumer-GPU). See Run-3 addendum below. |
| 13 | `CYFRAGOVPL/PLLuM-12B-chat` | 12B | BF16 | 1 or 2 | ready | Mistral base. AWQ-4bit variant `mozarcik/PLLuM-12B-chat-2512-awq` published 2026-05-26 (Run-3, consumer-GPU). See Run-3 addendum below. |
...
> **Run-3 addendum (2026-05-26) — consumer-GPU AWQ for #12 (8B Llama base) and #13 (12B Mistral base).** Following the Run-1 (Phase 2 v0.3 sweep, BF16 + AWQ on dual R9700) and Run-2 (Llama-PLLuM-70B AWQ family on AMD MI300X, eight variants published 2026-05-23) cycles, **Run-3** closed the consumer-GPU surface of the PLLuM family by quantizing the 8B-chat-2512 (Llama 3.1 base) and 12B-chat-2512 (Mistral base) checkpoints locally on R9700. Calibration corpus: shared `mozarcik/clinical-pl-smpc-awq-calibration` (418 Polish SmPC fragments, identical to Run-2). Tooling: `llm-compressor 0.10.0.2` in `~/venvs/autoawq/`, no cherry-picks required for Llama / Mistral architectures (gemma3 multimodal 4B variant explicitly out of Run-3 scope — needs `llm-compressor` main + PR #2571, deferred). Quantization walltime ~25 min/model on R9700 (vs ~2 h/model for the 70B family on MI300X). Output checkpoints: `mozarcik/Llama-PLLuM-8B-chat-2512-awq` (Llama 3.1 Community License compliance: `LICENSE` + `NOTICE` exact Meta wording + `USE_POLICY.md`) and `mozarcik/PLLuM-12B-chat-2512-awq` (Apache 2.0 — Mistral base, no Llama overlay). Gate 1 sanity: both 5/5 PASS via `/v1/completions` on the standard five Polish clinical prompts (2026-05-26 sanity JSON under `environment/sanity-tests/`). Engineering envelope: 8B at 5.53 GiB weights / 22.22 GiB KV cache / 88.89× max-concurrency on 1× R9700; 12B at 8.03 GiB weights / 19.77 GiB KV cache / 63.27× max-concurrency on 1× R9700. Phase 2 sweep for Run-3 variants is scoped for v0.5.0 — engineering envelope only is PUBLIC §11.1 here; per-N throughput / latency / W/tok numbers, when produced, will be EMBARGOED §11.2 (§11.3 for Polish models). Failure post-mortem (`hf download` shard truncation under `hf_transfer` accelerated mode, snap-Obsidian `XDG_CACHE_HOME` leak) recorded in `logbook/2026-05-26.md`.
...
### 5.3 Precision-ablation sub-study (same-checkpoint BF16 vs AWQ)

**Goal.** Isolate the effect of 4-bit AWQ quantization *at fixed model identity* — the only quantization comparison that Limitation 9 (§10) admits as valid. The main suite deliberately mixes precisions across *different* models (envelope characterization, not quantization-vs-quantization; Limitation 9). This sub-study instead holds the checkpoint constant and varies only the precision, so any throughput / latency / thermal / energy difference between the two members is attributable to the quantization format and its kernel path on gfx1201 (the kernel-efficiency gap documented in §5.2 errata (2)), not to a model-identity confound.

**Design.** Three same-checkpoint BF16 ↔ AWQ pairs, chosen because **both** precisions fit the 2× R9700 envelope locally (unlike the 70B family, where BF16 132 GB is not deployable — §4.3 errata):

| Pair | BF16 member | AWQ member |
|---|---|---|
| Bielik 4.5B v3.0 | `bielik-4.5b-v30` | `bielik-4.5b-v30-awq` |
| Llama-PLLuM 8B | `pllum-8b` | `pllum-8b-awq` |
| PLLuM 12B (Mistral) | `pllum-12b` | `pllum-12b-awq` |

**Procedure.** Each member is measured under the full Tier-A protocol (§7.4): the canonical 7-point ladder N ∈ {10, 25, 50, 100, 200, 500, 1000} (`CANON_N`), REPS = 10, at both TP = 1 and TP = 2 — 140 runs per cell. Entry to the ladder is gated by an **envelope + Gate-1 pre-check** (`benchmarks/results/_ablation_envelope_gate1/`): the AWQ checkpoint's load envelope is probed and a five-prompt Polish clinical Gate-1 coherence screen (auto-coherent, awaiting human review) is recorded before GPU-hours are committed to the full sweep. Within a pair, the BF16 and AWQ members share identical workload (§6), instrumentation and schema (§7.1), and cooldown discipline (§5.2), so their only controlled difference is precision. Orchestration logs live under `benchmarks/results/_ablation_{bielik45_awq,pllum8b_pair,pllum12b_pair}_logs/`; per-member outputs follow the standard Phase-2 layout (`benchmarks/results/<model>[-awq]/thermal-runs/`).

**Firmware (per §2.1).** These Tier-A ablation runs execute on the updated (BIOS 2202) platform and record `bios_version` live per run. Firmware-consistency is therefore per pair and read from the record, not assumed: the `pllum-8b` and `pllum-12b` pairs are measured with **both** members freshly on 2202 (firmware-consistent); the `bielik-4.5b` pair reuses its pre-update **BF16** member (BIOS 1715) against a freshly measured AWQ member (2202), so that pair inherits the §2.1 cross-firmware provenance caveat until its BF16 member is re-measured on 2202. Flag the mismatch per pair from the recorded `bios_version`.

**Embargo.** The design and the fixed-identity rationale are PUBLIC (§11.1); the per-cell throughput / latency / W/tok values and the rendered BF16-vs-AWQ figures are EMBARGOED (§11.2, stricter §11.3 for these Polish models).
...
- **Plots (default: scaling line + band).** The primary presentation is a **scaling line plot** — median of the metric vs N (log-x), connected per TP, with a shaded **min–max band** over the n = 10 reps (`benchmarks/scripts/plotting/plot_scaling_band.py`). This is the default because run-to-run variance is typically very small (CV ≈ 1 %), which collapses a box-and-whisker below marker size and overlaps the points; the connected median line shows the scaling trend (knee / plateau) while the thin band shows reproducibility honestly. A **box-and-whisker** alternative (`plot_boxwhisker.py`; box = IQR, whiskers = 1.5 × IQR (Tukey), all reps overlaid) is retained for metrics or cells whose spread is large enough that the distribution shape matters. Either way, each marker / box is the **median of n = 10 reps** at that N, rendered from `phase2_sweep_raw.csv`.
