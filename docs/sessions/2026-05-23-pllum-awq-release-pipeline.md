...
Both Bielik passes ran entirely on local hardware (single R9700, AWQ kernels on gfx1201 are slow — [EMBARGOED §11.3], multi-minute per call). No cloud LLM was used for the Polish text. Cleanup via `scripts/kill_port.sh 8100` (NOT `pkill -f`, per the project's process-isolation convention).
