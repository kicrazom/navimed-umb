...
| CONSIDER-4 | `eval-rag/` is `BLOCKED` on reviewer responses from Białas + Radliński since 2026-05-24. If 4-week no-response policy (`eval-rag/README.md:147`) fires (2026-06-21), the sub-project should either move to second-tier reviewers or be parked in `paused` status across MOC / project hub. | minor — calendar follow-up |
