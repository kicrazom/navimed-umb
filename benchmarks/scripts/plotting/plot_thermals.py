...
def plot(samples: list[dict], events: list[dict], out: Path) -> None:
    ...
    fig, (ax_temp, ax_use) = plt.subplots(
        2, 1, figsize=(13, 9), sharex=True, gridspec_kw={"height_ratios": [1.2, 1]}
    )
    ...
    for idx in sorted(gpus.keys()):
        ...
        if g["is_igpu"]:
            ...
            ls = "-"
        ax_temp.plot(
            g["t"], g["temp"], ls, color=color, linewidth=1.8, label=label, alpha=0.9
        )
    ...
    ax_temp.set_title(
        "System thermals during vLLM benchmark", fontsize=13, fontweight="bold", pad=10
    )
    ...
    for idx in sorted(gpus.keys()):
        ...
        ax_use.plot(
            g["t"], uses, ls, color=color, linewidth=1.6, label=label, alpha=0.85
        )
