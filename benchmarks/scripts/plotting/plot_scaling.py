...
def plot(data: dict, out_path: Path) -> None:
    ...
    if len(tp2_n) > 1:
        ax1.plot(
            ...
            linewidth=2.5,
            markersize=9,
            label="TP=2 (2 GPUs, tensor parallel)",
            zorder=3,
        )
    else:
        ax1.plot(
    ...
    ax1.set_title(
        "vLLM Throughput Scaling — AMD Radeon AI PRO R9700",
        fontsize=13,
        fontweight="bold",
        pad=14,
    )
    ...
    ax1.axvline(
        x=tp1_max_conc, color=color_tp1, linestyle=":", alpha=0.5, linewidth=1.5
    )
    ax1.text(
        tp1_max_conc,
        ax1.get_ylim()[1] * 0.05,
        ...
        ha="center",
        bbox=dict(
            facecolor="white", edgecolor=color_tp1, alpha=0.85, boxstyle="round,pad=0.3"
        ),
    )
    ax1.axvline(
        x=tp2_max_conc, color=color_tp2, linestyle=":", alpha=0.5, linewidth=1.5
    )
    ax1.text(
        tp2_max_conc,
        ax1.get_ylim()[1] * 0.05,
        ...
        ha="center",
        bbox=dict(
            facecolor="white", edgecolor=color_tp2, alpha=0.85, boxstyle="round,pad=0.3"
        ),
    )
    ...
    fig.text(0.5, 0.01, footer, ha="center", fontsize=8, color="#555555", wrap=True)
