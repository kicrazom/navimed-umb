...
MAX_LEN=8192
UTIL=0.90
N_LADDER=(500 1000)         # HIGH-N extension: {5..250} already done by run_bielik_tierA_n10_sweep.sh
TP_LADDER=(1 2)
REPS=10
COOLDOWN_S=30
...
PROGRESS_LOG="/tmp/bielik-tierA-highN-sweep-progress.log"
GLOBAL_LOG_DIR="$REPO_ROOT/benchmarks/results/_bielik_tierA_highN_logs"
...
write_embargo_header() {
    local f="$1"
    {
        ...
        echo "# Sweep: Bielik Tier-A n=10 HIGH-N {11b-v23,4.5b-v30,pl-11b-v30,11b-v30-awq},"
        echo "#        TP={1,2}, N={500,1000}, reps=10  (extends {5..250} → uniform 8-pt ladder)"
...
log "============================================================"
log "Phase 2 sweep — Bielik Tier-A n=10 HIGH-N {500,1000} (Polish single-shot variants)"
...
for ((m=0; m<${#MODEL_KEYS[@]}; m++)); do
    ...
    # IMPORTANT: by the time high-N runs (chained AFTER run_bielik_tierA_n10_sweep.sh
    # SWEEP_COMPLETE), the {5..250} reps already exist in the SAME OUT_DIR, so we
    # finalize over the FULL uniform 8-point ladder to produce a complete SUMMARY.
    if [[ -f "$FINALIZE" ]]; then
        log "Finalizing $KEY (full 8-pt ladder) ..."
        if python3 "$FINALIZE" \
            ...
            --max-len "$MAX_LEN" \
            --util "$UTIL" \
            --n-ladder 5,10,25,50,100,250,500,1000 \
            >> "$ORCHESTRATOR_LOG" 2>&1
        then
