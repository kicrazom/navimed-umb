...
PROGRESS_LOG="/tmp/bielik-tierA-sweep-progress.log"
GLOBAL_LOG_DIR="$REPO_ROOT/benchmarks/results/_bielik_tierA_logs"
...
write_embargo_header() {
    local f="$1"
    {
        ...
        echo "# Sweep: Bielik Tier-A n=10 {11b-v23,4.5b-v30,pl-11b-v30,11b-v30-awq},"
        echo "#        TP={1,2}, N={5,10,25,50,100,250}, reps=10"
...
for ((m=0; m<${#MODEL_KEYS[@]}; m++)); do
    ...
    # n-ladder/tp-ladder/reps left at finalize defaults (5,10,25,50,100,250 /
    # 1,2 / 10) which match this sweep exactly.
    if [[ -f "$FINALIZE" ]]; then
        log "Finalizing $KEY ..."
        if python3 "$FINALIZE" \
