#!/usr/bin/env bash
# Precision-ablation sweep — PLLuM-8B same-checkpoint pair (PLAN-2026-06-30, Faza 3,
# split #1). Llama-PLLuM-8B-chat-2512: BF16 (pllum-8b) then AWQ (pllum-8b-awq).
...
LOG_DIR="$REPO/benchmarks/results/_ablation_pllum8b_pair_logs"
...
CELLS=("pllum-8b bf16" "pllum-8b-awq awq")
...
done_n=0
echo "[$(date -Is)] ABLATION pllum-8b pair sweep START — $TOTAL runs (${#CELLS[@]} cells)" | tee -a "$LOG"
...
echo "[$(date -Is)] ABLATION pllum-8b pair sweep DONE" | tee -a "$LOG"
