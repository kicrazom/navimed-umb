#!/usr/bin/env bash
# NaviMed-UMB Phase 2 sweep — Run-3 consumer-GPU AWQ models, n=10 reps/cell.
#
# Two models, both compressed-tensors pack-quantized W4A16 (AWQ), local on
# 2× R9700 (gfx1201), TP={1,2}:
#   1. pllum-8b-awq   → Llama-PLLuM-8B-chat-2512-awq  (Llama 3.1 base, 5.5 GiB)
#   2. pllum-12b-awq  → PLLuM-12B-chat-2512-awq        (Mistral-Nemo base, 8.0 GiB)
#
# Per METHODOLOGY.md §5.2, mirroring run_bielik_11b_v30_sweep.sh (same N ladder,
# reps, cooldown, thermal sampling) for cross-model comparability:
#   - N ladder:  {5, 10, 25, 50, 100, 250}
#   - TP ladder: {1, 2}
#   - Reps/cell: 10                         (statistical n for medians/p99)
#   - Cooldown:  30s between runs
#   - max_model_len 8192, gpu_memory_utilization 0.90  (== bielik-11b-v30 cell)
#   - Workload: METHODOLOGY §6 standard (8 templates × 20 topics, max_tokens=128)
#   - Background thermal sampling at 1 Hz
#   - 8B sweep fully completes before 12B begins (sequential, partial results
#     available at the midpoint)
#
...
#     awq-tp{1,2}-n{5,10,25,50,100,250}-r{00..09}-{bench.log,events.json,thermals.jsonl,thermals.png}
#
...
#   systemd-inhibit --what=sleep:idle --who=navimed-run3 --why="overnight sweep" \
#       --mode=block bash benchmarks/scripts/orchestrators/run_pllum_run3_sweep.sh
...
# Parallel arrays: model key (bench/run_concurrent table) → result dir → HF name.
...
PROGRESS_LOG="/tmp/run3-sweep-progress.log"
GLOBAL_LOG_DIR="$REPO_ROOT/benchmarks/results/_run3_orchestrator_logs"
...
write_embargo_header() {
    local f="$1"
    {
        ...
        echo "# Sweep: Run-3 AWQ {8B,12B}, TP={1,2}, N={5,10,25,50,100,250}, reps=10"
...
# memory feedback_kill_isolation)
# ===========================================================================
...
log "============================================================"
log "Phase 2 sweep — Run-3 consumer-GPU AWQ (8B + 12B)"
...
for ((m=0; m<${#MODEL_KEYS[@]}; m++)); do
    ...
    log "################  MODEL: $KEY → $RESULT_DIR  ################"
    progress "MODEL_BEGIN key=$KEY result_dir=$RESULT_DIR"
